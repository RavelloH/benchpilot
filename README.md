...
The command tree covers configuration, adapters/devices, systems, runs, locks and approvals. Device and adapter commands are resolved from the configured instance and registered adapter: adding an in-process adapter registration does not require a CLI routing change. Use `benchpilot help --all --json` for discoverable machine-readable help. Every implemented command emits one `benchpilot.result` v3 object for `--json`, and a `benchpilot.event` v3 frame stream for `--jsonl`; the final event embeds the same Result object. RLog remains responsible for business logs, captures, and Run audit files rather than public JSONL stdout.
