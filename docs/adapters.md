...
内置的 `esp-idf` 是当前唯一的稳定适配器。实验性的 `ti-uniflash` 通过 DSLite 为具有有效 CCXML 的 TI 目标烧录预构建镜像，详见 [TI UniFlash 适配器](adapters/ti-uniflash.md)。创建新适配器应从 [适配器模板](adapter-template.md) 开始。
