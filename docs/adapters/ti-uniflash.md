...
`status` 与 `flash` 现在已由通用 DSLite 语义实现并经过 XDS110 硬件验证。Format v1 现支持 Capability 的 `availability_when` 声明：Runtime 在构建设备命令目录时使用设备配置判断该条件。因而 `run`、`stop`、`logs`、`console` 与 `send` 只会在配置 `monitor_port` 时出现；没有用户 UART 的纯 JTAG/SWD 目标不会暴露必然失败的串口命令。

串口会话保留 DTR/RTS 并使用标准的 BenchPilot 会话锁、Run、日志和停止生命周期。`info`、`reset`、`erase` 仍需要按 DSLite target-family 验证其输出和效果；`build`、`clean`、`fullclean`、`size` 与 `deploy` 属于独立 TI 工具链适配，而非 UniFlash。
