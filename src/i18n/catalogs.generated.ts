// Generated by scripts/generate-i18n.mjs. Do not edit manually.
export const catalogs = {
  en: {
    "cli.interaction.agent":
      "This command requires an interactive human session and cannot be run by an agent or with --non-interactive.",
    "cli.interaction.cancelled": "Interaction cancelled.",
    "cli.interaction.machine":
      "This command requires interactive input and cannot be used with machine output.",
    "cli.interaction.terminal":
      "This command requires an interactive terminal.",
    "error.approval": "Approval error: {message}",
    "error.configuration": "Configuration error: {message}",
    "error.device": "Device error: {message}",
    "error.interaction": "Interaction error: {message}",
    "error.internal": "Internal error: {message}",
    "error.lock": "Lock error: {message}",
    "error.operation": "Operation error: {message}",
    "error.system": "System error: {message}",
    "error.unknown": "Command failed: {message}",
    "error.usage": "Usage error: {message}",
    "help.arguments": "ARGUMENTS",
    "help.argumentsText": "See command path.",
    "help.command.adapter": "Inspect an adapter",
    "help.command.adapters": "List adapters",
    "help.command.approval": "Inspect an approval",
    "help.command.approvals": "List approvals",
    "help.command.clear-stale": "Remove stale locks",
    "help.command.config": "Read, explain, validate and edit configuration",
    "help.command.device": "Operate a device",
    "help.command.devices": "List devices",
    "help.command.doctor": "Check local environment",
    "help.command.explain": "Explain value provenance",
    "help.command.get": "Get a resolved configuration value",
    "help.command.help": "Show help",
    "help.command.init": "Initialize a project",
    "help.command.list": "List resources",
    "help.command.lock": "Inspect a lock",
    "help.command.locks": "List locks",
    "help.command.prune": "Remove old run records",
    "help.command.resolved": "Show resolved configuration",
    "help.command.run": "Inspect a run",
    "help.command.runs": "List runs",
    "help.command.scan": "Discover registered adapter devices",
    "help.command.set": "Set a configuration value",
    "help.command.system": "Operate a system",
    "help.command.systems": "List systems",
    "help.command.unset": "Unset a configuration value",
    "help.command.validate": "Validate configuration",
    "help.command.version": "Show version information",
    "help.commands": "Commands",
    "help.configuration": "CONFIGURATION",
    "help.configurationText":
      "global, project, local, explicit file and BENCHPILOT_* variables are merged.",
    "help.description": "DESCRIPTION",
    "help.descriptionText":
      "Command definitions drive parsing, help, validation, safety and execution.",
    "help.errorKinds": "ERROR KINDS",
    "help.examples": "EXAMPLES",
    "help.exitCodes": "EXIT CODES",
    "help.exitCodesText":
      "0 success; 2 usage; 3 configuration/resource; 4 lock; 5 operation; 6 timeout; 7 safety; 8 internal.",
    "help.globalOptions": "Global options",
    "help.group.adapter": "Inspect an adapter.",
    "help.group.adapters": "List installed adapter definitions.",
    "help.group.approval": "Inspect or resolve one human approval request.",
    "help.group.approvals": "List local human approval requests.",
    "help.group.config":
      "Read, explain, validate and safely edit configuration.",
    "help.group.device":
      "Operate a configured device through its capabilities.",
    "help.group.devices": "List configured or discovered devices.",
    "help.group.lock": "Inspect or clear one lock.",
    "help.group.locks": "Inspect physical-resource locks.",
    "help.group.root": "Agent-first device lifecycle CLI. Made by RavelloH.",
    "help.group.run": "Inspect a recorded operation.",
    "help.group.runs": "List and prune immutable operation records.",
    "help.group.system": "Orchestrate a configured system.",
    "help.group.systems": "List configured systems.",
    "help.group.version": "Show version information.",
    "help.more": "Run '{command} --help' for complete help.",
    "help.name": "NAME",
    "help.options": "OPTIONS",
    "help.output": "OUTPUT",
    "help.outputText":
      "Human summary by default; --json result or --jsonl structured events.",
    "help.safety": "SAFETY",
    "help.seeAlso": "SEE ALSO",
    "help.synopsis": "SYNOPSIS",
    "help.usage": "Usage",
    "help.workflow": "WORKFLOW",
    "help.workflowText":
      "Configuration → adapter → capability → operation runner → run record.",
    "init.done": "Initialized BenchPilot project.",
    "init.projectId": "Project ID",
    "init.projectName": "Project name",
    "menu.action.approve": "Approve",
    "menu.action.artifacts": "Show artifacts",
    "menu.action.clear": "Clear",
    "menu.action.clear-stale": "Clear stale locks",
    "menu.action.doctor": "Check environment",
    "menu.action.explain": "Explain configuration source",
    "menu.action.get": "Read configuration",
    "menu.action.info": "Show information",
    "menu.action.inspect": "Inspect",
    "menu.action.list": "List",
    "menu.action.logs": "Show logs",
    "menu.action.prune": "Prune",
    "menu.action.reject": "Reject",
    "menu.action.resolved": "Show resolved configuration",
    "menu.action.scan": "Discover devices",
    "menu.action.set": "Write configuration",
    "menu.action.show": "Show details",
    "menu.action.unset": "Delete configuration",
    "menu.action.validate": "Validate configuration",
    "menu.choose": "Choose the next action",
    "menu.config.existing": "Select an existing key",
    "menu.config.new": "Enter a new key",
    "menu.field.key": "configuration key",
    "menu.field.value": "configuration value",
    "menu.invalid": "Choose one of the listed options.",
    "menu.runs.all": "Remove all run records",
    "menu.runs.keep": "Keep newest runs",
    "menu.runs.older-than": "Remove runs older than a duration",
    "menu.value": "Enter {name}",
    "screen.root.adapter": "Inspect one adapter",
    "screen.root.adapters": "Installed adapters",
    "screen.root.approval": "Inspect or resolve one approval",
    "screen.root.approvals": "Human approval requests",
    "screen.root.commonOptions": "Common options",
    "screen.root.config": "Project configuration",
    "screen.root.configure": "Configure and discover",
    "screen.root.devices": "Configured or discovered devices",
    "screen.root.execute": "Execute",
    "screen.root.getStarted": "Get started",
    "screen.root.help": "Help",
    "screen.root.lock": "Inspect or clear one lock",
    "screen.root.locks": "Physical-resource locks",
    "screen.root.more": "More: benchpilot <command> --help",
    "screen.root.records": "Records and safety",
    "screen.root.run": "Inspect one operation record",
    "screen.root.runs": "Operation records and artifacts",
    "screen.root.systems": "Configured systems",
  },
  "zh-CN": {
    "cli.interaction.agent":
      "此命令需要人类交互会话，Agent 或 --non-interactive 模式不应直接调用。",
    "cli.interaction.cancelled": "交互已取消。",
    "cli.interaction.machine":
      "此命令需要交互式输入，不能与机器输出模式一起使用。",
    "cli.interaction.terminal": "此命令需要可交互的终端。",
    "error.approval": "审批错误：{message}",
    "error.configuration": "配置错误：{message}",
    "error.device": "设备错误：{message}",
    "error.interaction": "交互错误：{message}",
    "error.internal": "内部错误：{message}",
    "error.lock": "锁错误：{message}",
    "error.operation": "操作错误：{message}",
    "error.system": "系统错误：{message}",
    "error.unknown": "命令失败：{message}",
    "error.usage": "用法错误：{message}",
    "help.arguments": "参数",
    "help.argumentsText": "请参阅命令路径。",
    "help.command.adapter": "查看适配器",
    "help.command.adapters": "列出适配器",
    "help.command.approval": "查看审批",
    "help.command.approvals": "列出审批",
    "help.command.clear-stale": "删除过期锁",
    "help.command.config": "读取、解释、校验和编辑配置",
    "help.command.device": "操作设备",
    "help.command.devices": "列出设备",
    "help.command.doctor": "检查本地环境",
    "help.command.explain": "解释配置值来源",
    "help.command.get": "读取最终配置值",
    "help.command.help": "显示帮助",
    "help.command.init": "初始化项目",
    "help.command.list": "列出资源",
    "help.command.lock": "查看锁",
    "help.command.locks": "列出锁",
    "help.command.prune": "删除旧操作记录",
    "help.command.resolved": "显示最终配置",
    "help.command.run": "查看操作记录",
    "help.command.runs": "列出操作记录",
    "help.command.scan": "发现注册适配器的设备",
    "help.command.set": "写入配置值",
    "help.command.system": "操作系统",
    "help.command.systems": "列出系统",
    "help.command.unset": "删除配置值",
    "help.command.validate": "校验配置",
    "help.command.version": "显示版本信息",
    "help.commands": "命令",
    "help.configuration": "配置",
    "help.configurationText":
      "会合并全局、项目、本地、显式文件和 BENCHPILOT_* 环境变量。",
    "help.description": "说明",
    "help.descriptionText": "命令定义统一驱动解析、帮助、校验、安全和执行。",
    "help.errorKinds": "错误类型",
    "help.examples": "示例",
    "help.exitCodes": "退出码",
    "help.exitCodesText":
      "0 成功；2 用法；3 配置或资源；4 锁；5 操作；6 超时；7 安全；8 内部错误。",
    "help.globalOptions": "全局选项",
    "help.group.adapter": "查看一个适配器。",
    "help.group.adapters": "列出已安装的适配器定义。",
    "help.group.approval": "查看或处理一项人工审批请求。",
    "help.group.approvals": "列出本地人工审批请求。",
    "help.group.config": "读取、解释、校验并安全编辑配置。",
    "help.group.device": "通过能力操作一个已配置设备。",
    "help.group.devices": "列出已配置或已发现的设备。",
    "help.group.lock": "查看或清除一个锁。",
    "help.group.locks": "查看物理资源锁。",
    "help.group.root": "面向 Agent 的设备生命周期 CLI。Made by RavelloH。",
    "help.group.run": "查看一条操作记录。",
    "help.group.runs": "列出和清理不可变操作记录。",
    "help.group.system": "编排一个已配置系统。",
    "help.group.systems": "列出已配置的系统。",
    "help.group.version": "显示版本信息。",
    "help.more": "运行“{command} --help”查看完整帮助。",
    "help.name": "名称",
    "help.options": "选项",
    "help.output": "输出",
    "help.outputText":
      "默认输出人类摘要；--json 输出结果，--jsonl 输出结构化事件。",
    "help.safety": "安全",
    "help.seeAlso": "另请参阅",
    "help.synopsis": "概要",
    "help.usage": "用法",
    "help.workflow": "工作流",
    "help.workflowText": "配置 → 适配器 → 能力 → 操作运行器 → 操作记录。",
    "init.done": "BenchPilot 项目已初始化。",
    "init.projectId": "项目 ID",
    "init.projectName": "项目名称",
    "menu.action.approve": "批准",
    "menu.action.artifacts": "显示产物",
    "menu.action.clear": "清除",
    "menu.action.clear-stale": "清除过期锁",
    "menu.action.doctor": "检查环境",
    "menu.action.explain": "解释配置来源",
    "menu.action.get": "读取配置",
    "menu.action.info": "显示信息",
    "menu.action.inspect": "查看",
    "menu.action.list": "列出",
    "menu.action.logs": "显示日志",
    "menu.action.prune": "清理",
    "menu.action.reject": "拒绝",
    "menu.action.resolved": "显示最终配置",
    "menu.action.scan": "发现设备",
    "menu.action.set": "写入配置",
    "menu.action.show": "显示详情",
    "menu.action.unset": "删除配置",
    "menu.action.validate": "校验配置",
    "menu.choose": "选择下一步操作",
    "menu.config.existing": "选择已有配置键",
    "menu.config.new": "输入新配置键",
    "menu.field.key": "配置键",
    "menu.field.value": "配置值",
    "menu.invalid": "请选择列出的选项之一。",
    "menu.runs.all": "删除全部操作记录",
    "menu.runs.keep": "保留最新操作记录",
    "menu.runs.older-than": "删除早于指定时长的操作记录",
    "menu.value": "请输入{name}",
    "screen.root.adapter": "查看一个适配器",
    "screen.root.adapters": "已安装适配器",
    "screen.root.approval": "查看或处理一项审批",
    "screen.root.approvals": "人工审批请求",
    "screen.root.commonOptions": "常用选项",
    "screen.root.config": "项目配置",
    "screen.root.configure": "配置与发现",
    "screen.root.devices": "已配置或已发现设备",
    "screen.root.execute": "执行操作",
    "screen.root.getStarted": "开始使用",
    "screen.root.help": "帮助",
    "screen.root.lock": "查看或清除一个锁",
    "screen.root.locks": "物理资源锁",
    "screen.root.more": "更多：benchpilot <command> --help",
    "screen.root.records": "记录与安全",
    "screen.root.run": "查看一条操作记录",
    "screen.root.runs": "操作记录与产物",
    "screen.root.systems": "已配置系统",
  },
} as const;

export type Locale = keyof typeof catalogs;
export type MessageKey = keyof typeof catalogs.en;
